# homebridge-willow-mower

Homebridge plugin that exposes the **Willow Mower** to **HomeKit / Matter** via the mower's local REST API — no cloud required.
...
```bash
# From the homebridge-willow-mower directory:
npm install
npm run build
...
```bash
npm install -g github:JurgenLB/willow_mower
```
...
|---|---|---|---|
| `platform` | string | **required** | Must be `"WillowMower"` |
| `name` | string | `"Willow Mower"` | Display name in HomeKit |
...
| `GET /api/system/hardwareInfo` | Robot indentification |
